      ...
      else if ((ADD_CRC_CHECK && check_CRC(_inputString)) || !ADD_CRC_CHECK) // Only need to check CRC if ADD_CRC_CHECK flag is high:
      {
        ...
        if (_inputString.charAt(2) == 'I' && isDigit(_inputString.charAt(3)))
        {
          ...
          if (_unit_id == (int)_UNIT_ID)
          {
            ...
            else if (_inputString.charAt(4) == 'S' && _inputString.charAt(5) == 'S' && _inputString.charAt(6) == 'T' && _inputString.charAt(7) == 'C'&& _inputString.charAt(8) == 'O'&& _inputString.charAt(9) == 'N'&& _inputString.charAt(10) == 'O')
            {
              // Switch on or off the temp conversion
              // Need to check the next chars:
              if(_inputString.charAt(11) == 'N')
              {
                solar_tempcon_flag = true;
              }
              else if(_inputString.charAt(11) == 'F' &&_inputString.charAt(12) == 'F')
              {
                solar_tempcon_flag = false;
              }
              temp_con_status_flag = true;
              data_sent_flag = true; // This flag allows the main loop to then check out the correct data value and return it.
            }
            else if (_inputString.charAt(4) == 'S' && _inputString.charAt(5) == 'S' && _inputString.charAt(6) == 'T' && _inputString.charAt(7) == 'C'&& _inputString.charAt(8) == 'O'&& _inputString.charAt(9) == 'N'&& _inputString.charAt(10) == 'p')
            {
              // We get here is asking for the conversion values to be returned
              // If we ask for "aaI0SSTCONp0.18r25.0#" then it will set the conversion values for p and r
              // So need to find the data between p & r and between r and #
              // Then convert the data from string to a float
              String data_substring;
              int start_index;
              int end_index;

              start_index = _inputString.indexOf('p');
              end_index = _inputString.indexOf('r', start_index + 1);
              data_substring = _inputString.substring(start_index + 1, end_index);
              solar_tempcon_percentage = data_substring.toFloat();
              start_index = end_index;
              end_index = _inputString.indexOf('#', start_index + 1);
              data_substring = _inputString.substring(start_index + 1, end_index);
              solar_tempcon_rated_c = data_substring.toFloat();
              temp_con_set_flag = true;
              
              data_sent_flag = true; // This flag allows the main loop to then check out the correct data value and return it.
            }            
            else if (_inputString.charAt(4) == 'S' && _inputString.charAt(5) == 'S' && _inputString.charAt(6) == 'T' && _inputString.charAt(7) == 'C'&& _inputString.charAt(8) == 'O'&& _inputString.charAt(9) == 'N')
            {
              // We get here is asking for the conversion values to be returned
              // We dont need the average number
              temp_con_return_flag = true;
              data_sent_flag = true; // This flag allows the main loop to then check out the correct data value and return it.
            }  
