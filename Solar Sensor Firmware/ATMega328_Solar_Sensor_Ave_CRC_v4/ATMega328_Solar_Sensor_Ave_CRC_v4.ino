/*
  ...
  Then convert to an INT -> analogWrite(ANALOG_OUT_PIN, PWM_float);
...
void t1SCallback() {
  if (!t1S.isFirstIteration())
  {
    ...
    // ******** OUTPUT ANALOG IRRADIANCE VALUE ****************//
    // Here we write the PWM to the output. from 0-255
    // Want to round up/down as needed (so 12.8 becomes 13 NOT 12).
    // To do this we add 0.5 to the value and then cast to int. This is simple.
    float PWM_float = (( 256.0 * solar_data.data_1s ) / (float)MAX_IRRADIANCE) + 0.5;
...
void setup() {
  ...
  // Get the m and c irradiance conversion data
  ...
  // Get the temperature conversion data (flag, rated C and percentage)
  // But if they are NAN then initialise the values
  EEPROM.get(TEMP_COMP_FLAG_LOC, solar_data.solar_tempcon_flag);
  //check for NAN
  if (isnan(solar_data.solar_tempcon_flag))
  {
    solar_data.solar_tempcon_flag = APPLY_TEMP_COMP_INIT;
    EEPROM.put(TEMP_COMP_FLAG_LOC, solar_data.solar_tempcon_flag);
  }
  EEPROM.get(TEMP_COMP_RATED_C_LOC, solar_data.solar_tempcon_rated_c);
  //check for NAN
  if (isnan(solar_data.solar_tempcon_rated_c))
  {
    solar_data.solar_tempcon_rated_c = RATED_TEMP_INIT;
    EEPROM.put(TEMP_COMP_RATED_C_LOC, solar_data.solar_tempcon_rated_c);
  }
  EEPROM.get(TEMP_COMP_PERCENTAGE_LOC, solar_data.solar_tempcon_percentage);
  //check for NAN
  if (isnan(solar_data.solar_tempcon_percentage))
  {
    solar_data.solar_tempcon_percentage = TEMP_COMP_VAL_INIT;
    EEPROM.put(TEMP_COMP_PERCENTAGE_LOC, solar_data.solar_tempcon_percentage);
  }
...
void loop()
{
  ...
  if (stringComplete == true)
  {
    ...
    else
    {
      ...
      else if (checkData.temp_con_status_flag == true)
      {
        solar_data.solar_tempcon_flag = checkData.solar_tempcon_flag;
        // store the new data to EEPROM:
        EEPROM.put(TEMP_COMP_FLAG_LOC, solar_data.solar_tempcon_flag); 
        returnString = START_STR;
        returnString += (String)UNIT_ID;
        if(solar_data.solar_tempcon_flag == true)
        {
          returnString += "SSTCONON";
        }
        else
        {
          returnString += "SSTCONOFF";
        }
        checkData.temp_con_status_flag = false;
        checkData.data_sent_flag = false;
      }
      else if (checkData.temp_con_set_flag == true)
      {
        // Return temperature conversion settings "aaI0SSTCONp0.18r25.0#"
        solar_data.solar_tempcon_percentage = checkData.solar_tempcon_percentage;
        solar_data.solar_tempcon_rated_c = checkData.solar_tempcon_rated_c;
        // Store this data to EEPROM
        EEPROM.put(TEMP_COMP_PERCENTAGE_LOC, solar_data.solar_tempcon_percentage);
        EEPROM.put(TEMP_COMP_RATED_C_LOC, solar_data.solar_tempcon_rated_c);
        returnString = START_STR;
        returnString += (String)UNIT_ID;
        returnString += "SSTCONp";
        returnString += (String)solar_data.solar_tempcon_percentage;
        returnString += "r";
        returnString += (String)solar_data.solar_tempcon_rated_c;
        checkData.temp_con_set_flag = false;
        checkData.data_sent_flag = false;
      }
      else if (checkData.temp_con_return_flag == true)
      {
        // Return temperature conversion settings "aaI0SSTCONp0.18r25.0#"
        returnString = START_STR;
        returnString += (String)UNIT_ID;
        ...
        returnString += "r";
        returnString += (String)solar_data.solar_tempcon_rated_c;
        checkData.temp_con_return_flag = false;
        checkData.data_sent_flag = false;
      }
